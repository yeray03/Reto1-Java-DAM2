...
public class HistoricoDAO extends FirebaseInitialize {
	 ...
	 */
	public ArrayList<Historico> getHistoricoPorUsuario(String nickname)
			throws ExecutionException, InterruptedException {
		Firestore db = FirestoreClient.getFirestore(FirebaseApp.getInstance());

		CollectionReference historicoRef = db.collection("usuarios").document(nickname).collection("historico");
	...
	public void addHistorico(String nickname, Historico historico) throws Exception {
		...
		db.collection("usuarios").document(nickname).collection("historico").document().set(historico);
